<template>
  <view v-if="show" class="t-wrapper"
    @touchmove.stop.prevent="moveHandle">
    <view class="t-mask" :class="{active:active}" @click.stop="close">
    </view>
        ...
        <view class="t-control-box__item">
          ...
          <view class="t-controller boxs"
            ...
            <view class="t-transparency">
              <view class="t-circle"
                :style="{ left: site[2].left - 12 + 'px' }"></view>
            </view>
          </view>
        </view>
      </view>
  ...
  export default {
    ...
    methods: {
      ...
      setPosition(x, y, index) {
        ...
        this.site[index].left = Math.max(0, Math.min(
          parseInt(x - left), width));
        if (index === 0) {
          this.site[index].top = Math.max(0, Math.min(
            parseInt(y - top), height));
          // 设置颜色
          this.hsb.s = parseInt((100 * this.site[index]
            .left) / width);
          this.hsb.b = parseInt(100 - (100 * this.site[
            index].top) / height);
          this.setColor();
          this.setValue(this.rgba);
        } else {
       ...
       */
      rgbToHex(rgb) {
        let hex = [rgb.r.toString(16), rgb.g.toString(16),
          rgb.b.toString(16)
        ];
      ...
      changeViewByHsb() {
        ...
        this.site[0].left = parseInt(this.hsb.s * a.width /
          100);
        this.site[0].top = parseInt((100 - this.hsb.b) * a
          .height / 100);
      ...
      rgbToHsb(rgb) {
        ...
        if (hsb.s != 0) {
          ...
          else if (rgb.g == max) hsb.h = 2 + (rgb.b - rgb
            .r) / delta;
        ...
        views
          ...
          .boundingClientRect(data => {
            if (!data || data.length === 0) {
              setTimeout(() => this.getSelectorQuery(),
                20)
              return
            }
            this.position = data;
